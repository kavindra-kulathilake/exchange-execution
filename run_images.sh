...
podman run -d  --name exchange-web  --network exchange -e REACT_APP_API_BASE_URL=http://api.sprintly-exchange.com  -p 3000:3000 docker.io/kkulathilake/exchange-web:latest-amd64
docker build -t kkulathilake/nginx-exchange:latest 
podman stop nginx-exchange
podman rm nginx-exchange
podman run -d   -p 8080:80  --network exchange --name nginx-exchange   localhost/kkulathilake/nginx-exchange:latest
